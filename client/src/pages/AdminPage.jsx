...
import { adminDownload, adminRequest, createAdminSession, getAdminSettings, updateSiteSettings, listAdminCommunityPosts, moderateCommunityPost } from "../api/http";
...
function Riders({ riders, adminKey, refresh }) {
  ...
  const toggleAll = () => {
    setSelectedIds(allSelected ? [] : riders.map((rider) => rider.id));
  };
  ...
  const matchRoster = async () => {
    ...
    try {
      const body = new FormData();
      body.append("roster_file", rosterFile);
      const result = await adminRequest("/registrations/roster-match", adminKey, {
        method: "POST",
        body,
      });
      setSelectedIds(result.matched_ids);
      setActionMessage(
        `${result.matched} participant${
          result.matched === 1 ? "" : "s"
        } matched from roster. ${result.unmatched} row${
          result.unmatched === 1 ? " was" : "s were"
        } not found.`
      );
    } catch (error) {
      setActionMessage(error.message);
    ...
    <section className="rounded-3xl border border-[#071313]/10 bg-white shadow-sm">
      <article className="rounded-t-3xl border-b border-[#071313]/10 bg-[#fbf8ef] p-5 sm:p-6">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-black/65">
          Upload a CSV, Excel, or text-based PDF attendance roster to select
          matching riders. Approve, check in, preview one certificate, then
          generate and send to selected participants.
        </p>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-3">
          <div className="rounded-xl border border-[#071313]/12 bg-white p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              >
                {rosterFile ? rosterFile.name : "Choose roster"}
              </label>
              <input
                id="roster-file"
                type="file"
          ...
          <div className="rounded-xl border border-[#071313]/12 bg-white p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={busy || !selectedIds.length}
                className="rounded-lg bg-[#071313] px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40"
              >
                {busy ? "Working..." : `Check in (${selectedIds.length})`}
              </button>
            </div>
          </div>
      ...
      <div className="overflow-x-auto rounded-b-3xl">
        <table className="w-full min-w-[920px] text-left text-sm">
          ...
          <tbody>
            {riders.length === 0 && (
              <tr>
                <td colSpan={8} className="p-6 text-center text-sm text-black/55">
            ...
            {riders.map((rider) => (
              <tr key={rider.id} className="border-b border-black/5">
                ...
                <td className="p-4 font-bold">{rider.full_name}</td>
                ...
                <td className="p-4">
                  <select
                    value={rider.status}
                    onChange={(event) =>
                      changeStatus(rider.id, event.target.value)
                    }
                    className="rounded-lg border border-black/20 bg-white p-2 text-sm"
                  >
                    {["pending", "approved", "checked_in", "cancelled"].map(
                      (status) => (
